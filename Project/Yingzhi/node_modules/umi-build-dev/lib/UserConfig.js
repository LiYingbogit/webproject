...
var _createRouteMiddleware = require("./middlewares/createRouteMiddleware");
...
function getConfigFile(cwd, service) {
  const printWarn = service.printWarn;

  const files = _constants.CONFIG_FILES.map(file => (0, _path.join)(cwd, file)).filter(file => (0, _fs.existsSync)(file));

  if (files.length > 1) {
    printWarn(`Muitiple config files ${files.join(', ')} detected, umi will use ${files[0]}.`);
  }
...
class UserConfig {
  static getConfig(opts = {}) {
    ...
    if (absConfigPath) {
      return normalizeConfig(_objectSpread({}, requireFile(absConfigPath), env ? requireFile(absConfigPath.replace(/\.js$/, `.${env}.js`)) : {}, isDev ? requireFile(absConfigPath.replace(/\.js$/, '.local.js')) : {}));
    } else {
      return {};
    }
  }
  ...
  getConfig(opts = {}) {
    ...
    const _this$service = this.service,
          ...
          printError = _this$service.printError;
...
\r\n${e.toString()}`;
    ...
    config = normalizeConfig(_objectSpread({}, requireFile(file, {
      onError
  ...
  watchWithDevServer() {
    ...
    this.watchConfigs((event, path) => {
      ...
      try {
        ...
        const oldConfig = (0, _lodash2.default)(this.config);
        this.config = newConfig;
        ...
        try {
          for (var _iterator3 = this.plugins[Symbol.iterator](), _step3; !(_iteratorNormalCompletion3 = (_step3 = _iterator3.next()).done); _iteratorNormalCompletion3 = true) {
            ...
            if (!(0, _lodash.default)(newConfig[name], oldConfig[name])) {
  ...
  watchConfigs(handler) {
    const env = process.env.UMI_ENV;
    const watcher = this.watch('CONFIG_FILES', (0, _lodash3.default)(_constants.CONFIG_FILES.map(file => [file, env ? [file.replace(/\.js$/, `.${env}.js`)] : [], file.replace(/\.js$/, `.local.js`)])));
